...
var easyquiz = /** @class */ (function () {
    ...
    // 预热语音合成引擎
    easyquiz.prototype.initSpeechSynthesis = function () {
        var _this = this;
        if ('speechSynthesis' in window) {
            // 创建一个静音的语音合成来预热引擎
            var warmupSpeech = new SpeechSynthesisUtterance('');
            warmupSpeech.lang = 'ja-JP';
            warmupSpeech.volume = 0; // 静音
            warmupSpeech.rate = 1;
            warmupSpeech.pitch = 1;
            // 监听语音合成事件
            warmupSpeech.onstart = function () {
                _this.speechReady = true;
                console.log('语音合成引擎已预热完成');
            };
            warmupSpeech.onerror = function (event) {
                console.warn('语音合成预热失败:', event);
                _this.speechReady = true; // 即使失败也标记为准备就绪，避免阻塞
            };
            warmupSpeech.onend = function () {
                _this.speechReady = true;
            };
            // 等待语音库加载完成后再预热
            ...
            // 设置超时，避免无限等待
            setTimeout(function () {
                if (!_this.speechReady) {
                    _this.speechReady = true;
                    console.log('语音合成预热超时，继续执行');
                }
            }, 2000);
        }
    };
    ...
    easyquiz.prototype.speak = function (word) {
        ...
        if ('speechSynthesis' in window) {
            ...
            if (typeof window['speechReady'] !== 'undefined' && !window['speechReady']) {
                setTimeout(function () { return _this.speak(word); }, 100);
                return;
            }
